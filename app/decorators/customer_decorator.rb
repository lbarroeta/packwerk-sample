...
module CustomerDecorator
  def self.index_dropdown
    [
      { title: I18n.t('create_client', scope: 'decorators.customer/index_dropdown'), url: '/planify/customers/new'  }
    ]
  end
  ...
  def self.table_dropdown(object)
    ...
    [
      { title: I18n.t('see_details', scope: 'decorators.customer/table_dropdown'), url: "/planify/customers/#{object.id}" },
      { title: I18n.t(status, scope: 'decorators.customer/table_dropdown'), url: "/planify/customers/#{object.id}", method: 'destroy'.to_sym }
    ]
  end
end
